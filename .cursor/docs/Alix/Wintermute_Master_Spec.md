...
## Recent Titans Memory Experiments (Dec 2025)
- Integrated lucidrains `titans-pytorch` MAC variant and trained on TinyStories (sampled).
- Best perplexity so far: 12.07 (loss 2.49) with dim 384, seq_len 512, lr floor 5e-5, 4k steps (MPS, batch 6).
- Baseline without long-term memory: ppl 13.15 at similar scale.
- Synthetic QA memory probe (fact + filler + “Answer:”): zero-shot on ckpt_step_4000 remains 0%; a 300-step QA-format finetune on ckpt_step_4000 improves to 21% accuracy (previous finetune pass landed at 18%), and 25.5% when allowing a 6-token decode span; baseline remains 0%.
- lucidrains repo is still WIP (open PRs/issues, no pretrained weights); expect to customize and validate behaviors.
...
### Potential Development Path: HRM Dual-Brain + Titan-Style Surprise Learning
- **Architecture:** Dual-brain control with a fast hierarchical planner (subgoal drafts) paired to a slow executor/critic (tool-using, step-by-step) plus a surprisal monitor that gates re-planning, retrieval, or tool calls when surprise is high.
- **Control Loop:** Plan → execute → measure surprisal/uncertainty at each step → if medium/high surprise, branch or re-plan, consult retrieval/tools, and log the episode; keep planned vs executed traces for diffing and updating priors.
- **Memory:** Episodic store keyed by surprise embeddings; semantic store for distilled learnings; scratchpad for the current trajectory; promote resolved, frequent surprises into stable priors.
- **Learning Signals:** Surprisal-prioritized replay; contrast pre/post-fix pairs; reward shaping that penalizes unresolved surprise and rewards efficient resolution; curiosity with safeguards to avoid thrash.
- **Planning Enhancements:** Beam/branch search when surprise spikes; tool grounding to cut uncertainty; use dual traces to refine planner priors and executor heuristics.
- **Safety/Guardrails:** Surprise-triggered safety checks before risky actions; rate-limit re-plans; decay thresholds as confidence improves.
- **Modes:** Online (surprise gating active, write episodic logs), Training (off-policy replay of surprise episodes to tune planner/executor/gates), Eval (report surprise hotspots and resolution paths).
- **Open Questions:** Calibrating surprise thresholds per domain; best keys for surprise memory (latent vs tool-augmented); balancing curiosity vs stability in long-running sessions.
