...
- **Local Titan runs with S3-backed inputs**: on this macOS host, activate `.venv_docs`, export `AWS_PROFILE=experimental-admin` and `AWS_SDK_LOAD_CONFIG=1`, then use `python -u ... | tee ...` so `boto3` sees SSO credentials and the log file is not stalled by stdout buffering.
- **Local Titan interpreter gotcha**: the shell's default `python3` may still point at the system interpreter even when a docs-oriented venv exists. For reproducible local Titan runs, prefer the explicit interpreter path `.venv_docs/bin/python`.
- **Titan workstation handoff note**: the checked-in Titan workflow is not tied to this macOS laptop, but local virtualenv paths are host-specific. On a new x86 Linux laptop, recreate the local Python environment there instead of copying `.venv_docs`; the canonical long-run execution path remains the AWS detached Ubuntu runner.
