...
def addWeighted(
    src1: cv2.GMat, alpha: float, src2: cv2.GMat, beta: float, gamma: float, ddepth: int = ...
) -> cv2.GMat: ...
def bilateralFilter(
    src: cv2.GMat, d: int, sigmaColor: float, sigmaSpace: float, borderType: int = ...
) -> cv2.GMat: ...
@_typing.overload
...
def cartToPolar(
    x: cv2.GMat, y: cv2.GMat, angleInDegrees: bool = ...
) -> tuple[cv2.GMat, cv2.GMat]: ...
@_typing.overload
...
@_typing.overload
def kmeans(
    data: cv2.GMat, K: int, criteria: cv2.typing.TermCriteria, attempts: int, flags: cv2.KmeansFlags
) -> tuple[cv2.GOpaqueT, cv2.GMat, cv2.GMat]: ...
@_typing.overload
def kmeans(
    data: cv2.GArrayT,
...
def normalize(
    src: cv2.GMat, alpha: float, beta: float, norm_type: int, ddepth: int = ...
) -> cv2.GMat: ...
@_typing.overload
def parseSSD(
    in_: cv2.GMat, inSz: cv2.GOpaqueT, confidenceThreshold: float = ..., filterLabel: int = ...
...
def polarToCart(
    magnitude: cv2.GMat, angle: cv2.GMat, angleInDegrees: bool = ...
) -> tuple[cv2.GMat, cv2.GMat]: ...
def remap(
    src: cv2.GMat,
