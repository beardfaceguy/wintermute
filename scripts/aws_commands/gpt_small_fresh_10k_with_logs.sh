#!/usr/bin/env bash
# Fresh GPT-small run to 10k steps with CloudWatch + S3 logging.
# Set INSTANCE_ID. Log group /aws/ssm/titan-llm-training (see iam/ssm_cloudwatch_logs_inline_policy.json).
...
LOG_PREFIX="ssm-logs/gpt-small-fresh10k/$(date +%Y%m%d%H%M%S)"
...
RUN_ID="gpt_small_fresh10k_$(date +%Y%m%d%H%M%S)"
...
echo "[meta] duration_sec=${DURATION_SEC}"
...
CMD_ID=$(AWS_PROFILE="${AWS_PROFILE}" aws ssm send-command \
  ...
  --comment "gpt-small fresh 10k + CloudWatch + S3 logs" \
